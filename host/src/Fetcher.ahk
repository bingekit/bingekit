...
AHK_StartSmartFetch(windowId, url, actionJs, callbackId, botCheckJs := "") {
    DoSmartFetch() {
        ...
        wrapperJs .= "    window.BK_EXPOSE_FETCHER = function(title, w, h) { window.chrome.webview.hostObjects['" hostObjName "'].UpdateWindow(true, title || 'Manual Debug Expose', w || 1000, h || 800).catch(e => console.error(e)); };`n"
...
AHK_StartRawFetchParse(windowId, url, actionJs, callbackId) {
    DoRawFetch() {
        ...
        try {
            ...
            wrapperJs .= "            result.then(res => {`n"
            wrapperJs .= "                window.chrome.webview.hostObjects['" hostObjName "'].ReturnData(JSON.stringify(res)).catch(e => console.error('[RawParseFetch Debug] COM Error:', e));`n"
            wrapperJs .= "            }).catch(e => {`n"
            wrapperJs .= "                window.chrome.webview.hostObjects['" hostObjName "'].ReturnError(JSON.stringify(e.message||e)).catch(e => console.error('[RawParseFetch Debug] COM Error:', e));`n"
            wrapperJs .= "            });`n"
            wrapperJs .= "        } else {`n"
            wrapperJs .= "            var jsonStr = typeof result === 'string' ? result : JSON.stringify(result);`n"
            wrapperJs .= "            console.log('[RawParseFetch Debug] Transmitting synchronous result...');`n"
            wrapperJs .= "            window.chrome.webview.hostObjects['" hostObjName "'].ReturnData(jsonStr).catch(e => console.error('[RawParseFetch Debug] COM Error:', e));`n"
            wrapperJs .= "        }`n"
            wrapperJs .= "    } catch(e) {`n"
            wrapperJs .= "        console.error('[RawParseFetch Debug] Exception executed crashed:', e);`n"
            wrapperJs .= "        window.chrome.webview.hostObjects['" hostObjName "'].ReturnError(JSON.stringify(e.message||e)).catch(err => console.error('[RawParseFetch Debug] COM Error:', err));`n"
            wrapperJs .= "    }`n"
            wrapperJs .= "});`n"
