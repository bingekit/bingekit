...
const fetchLiveEpisodes = async (item: FollowedItem, plugin: SitePlugin, credentials: CredentialItem[]) => {
  ...
  if (!authValid && plugin.auth?.loginUrl) {
     ...
     window.dispatchEvent(new CustomEvent('sv-live-episodes', { detail: { id: item.id, episodes: [] } }));
     return;
  }
  ...
  } catch(e) { console.error('Live episode fetch failed:', e); window.dispatchEvent(new CustomEvent('sv-live-episodes', { detail: { id: item.id, episodes: [] } })); }
};
