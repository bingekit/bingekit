...
export const ActivityView = () => {
  ...
  const [addTrackerState, setAddTrackerState] = React.useState<any>({ url: '', siteId: '', flowId: '', label: '', title: '', imgUrl: '', testerItems: null });
  const {
  ...
  React.useEffect(() => {
    const handleEpisodes = (e: any) => {
       ...
       if (e.detail?.id === expandedItemId) {
          setLiveEpisodes(e.detail.episodes || []);
       }
    };
    window.addEventListener('sv-live-episodes', handleEpisodes);
    return () => window.removeEventListener('sv-live-episodes', handleEpisodes);
  }, [expandedItemId]);
    ...
    <div className="p-8 max-w-6xl mx-auto w-full h-full overflow-y-auto no-scrollbar">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => {
              setAddTrackerState({ url: url, siteId: plugins[0]?.id || '', flowId: '', label: '', title: new URL(url).hostname, imgUrl: '' });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {(() => {
           ...
           return Array.from(grouped.values()).map(item => {
             ...
             const hasAdvancedTracking = primarySource.trackingFlowId || !!(plugin?.tracking?.listSel && plugin?.tracking?.itemSel);
          ...
          >
            ...
            <div className="flex-1 min-w-0 py-1">
              ...
              <p className="text-[10px] text-zinc-600 truncate mt-2 font-mono">
                {item._sources.length > 1 ? `${item._sources.length} Sources` : (plugins.find(p => p.id === item.siteId)?.name || 'Unknown Site')}
              </p>
            </div>
          </div>
        );
      ...
      {expandedItemId && (
        <Modal isOpen={true} title="Tracking Details" onClose={() => setExpandedItemId(null)}>
          {(() => {
            const item = followedItems.find(i => i.id === expandedItemId);
            if (!item) return null;
            return (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => {
                    if (confirm('Stop tracking this item?')) {
                      setFollowedItems(followedItems.filter(i => i.id !== expandedItemId));
                ...
                <div className="mt-6 border-t border-zinc-800 pt-4">
                  <h4 className="text-sm font-medium text-zinc-400 mb-3 flex items-center justify-between">
                    <span>Episodes ({(item.watchedEpisodes || []).length}/{liveEpisodes.length})</span>
                    {isFetchingEpisodes && <RefreshCw size={14} className="animate-spin text-zinc-500" />}
                  </h4>
                             ...
                             <div className="flex gap-2">
                               <button 
                                  onClick={() => {
                                      ...
                                      if (confirm('Mark this and all previous episodes as watched?')) {
                                          const toMark = liveEpisodes.slice(watchIdx).map(e => e.id); // Assuming array is chronological descending (newest first)
                                          // Wait usually lists are newest first so maybe it's slice(watchIdx)?
                                          // Or we just mark all
                                          toMark.forEach(id => {
                                  ...
                                  className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${isWatched ? 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700' : 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 mix-blend-screen'}`}
                               >
        ...
        <Modal isOpen={true} onClose={() => setIsAddingTracking(false)} title="Custom Tracking Info" width="600px">
           <div className="space-y-4">
               <div>
                  ...
                  <input value={addTrackerState.url} onChange={e => setAddTrackerState({...addTrackerState, url: e.target.value})} className="w-full bg-zinc-900 border border-zinc-800 text-sm p-2 rounded text-zinc-200 outline-none focus:border-emerald-500/50" placeholder="https://..." />
               </div>
               ...
               {plugins.length > 0 && (
                 <div className="flex gap-4">
                   <div className="flex-1">
                     ...
                     <select value={addTrackerState.siteId} onChange={e => setAddTrackerState({...addTrackerState, siteId: e.target.value, flowId: ''})} className="w-full bg-zinc-900 border border-zinc-800 text-sm p-2 rounded text-zinc-200 outline-none">
                   ...
                   <div className="flex-1">
                     ...
                     <select value={addTrackerState.flowId} onChange={e => setAddTrackerState({...addTrackerState, flowId: e.target.value})} className="w-full bg-zinc-900 border border-zinc-800 text-sm p-2 rounded text-zinc-200 outline-none">
                        <option value="">Default (or Auto-Match)</option>
                        {(() => {
                           const pl = plugins.find(p => p.id === addTrackerState.siteId);
                           if (!pl || !pl.trackingFlows) return null;
                           return pl.trackingFlows.map(f => <option key={f.id} value={f.id}>{f.name || f.id}</option>);
                        })()}
                     </select>
                   </div>
                 </div>
                     ...
                     <button
                        onClick={async () => {
                          ...
                          if (pl?.trackingFlows) {
                             ...
                             else if (pl.trackingFlows.length > 0) trackingConf = pl.trackingFlows[0]; // fallback
                          }
                          ...
                          const res = await window.SmartFetch(addTrackerState.url, js);
               ...
               <div className="flex justify-end pt-4 border-t border-zinc-800 gap-3">
                  ...
                  <button onClick={() => {
                      if (!addTrackerState.url || !addTrackerState.siteId) { alert("URL and Site ID are required"); return; }
                      setFollowedItems([
                         ...followedItems,
                         {
                            id: Date.now().toString(),
                            title: addTrackerState.title || new URL(addTrackerState.url).hostname,
                            label: addTrackerState.label || '',
                            url: addTrackerState.url,
...
const fetchLiveEpisodes = async (item: FollowedItem, plugin: SitePlugin) => {
  ...
  try {
     ...
     if (Array.isArray(results)) {
         window.dispatchEvent(new CustomEvent('sv-live-episodes', { detail: { id: item.id, episodes: results } }));
     }
  } catch(e) { console.error('Live episode fetch failed:', e); }
};
