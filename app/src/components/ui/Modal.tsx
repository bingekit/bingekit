...
export const Modal = ({ isOpen, onClose, title, children }: { isOpen: boolean, onClose: () => void, title: string, children: React.ReactNode }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[color-mix(in_srgb,var(--theme-bg)_80%,transparent)] backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[var(--theme-surface)] border border-[color-mix(in_srgb,var(--theme-text)_10%,transparent)] rounded-2xl w-full max-w-lg shadow-2xl relative flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
